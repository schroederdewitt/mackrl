...
class NStepRunner():
    ...
    def __init__(self, multiagent_controller=None,
        ...
        self.T_env = 0
        self.N_episode = 0 # number of episodes sampled
        ...
        envs_fn = partial(envs_fn, env_args = self.args.env_args, args=self.args)
        ...
        self.env_setup_info = self._exch_msgs(ids=range(0, self.batch_size), msgs=["SCHEME"]*self.batch_size, kwargs=kwargs)
    ...
    def _loop_worker(envs,
        ...
        if chosen_actions == "SCHEME":
            ...
            reward, terminated, env_info = \
                _env.step([int(_i) for _i in chosen_actions], obs_noise_std=obs_noise_std)
    ...
    def _main_thread_worker(self, ids, msgs, **kwargs):
        res = []
    ...
    def _exch_msgs(self, ids, msgs, **kwargs):
        """
        ...
        msgs_with_ids = list(zip(ids, msgs, output_buffers, column_schemes, [kwargs]*len(ids)))
    ...
    def reset(self, test_mode=False, **kwargs):
        ...
        self.hidden_states, self.hidden_states_format = self.multiagent_controller.generate_initial_hidden_states(self.batch_size, test_mode=test_mode)
    ...
    def step(self, actions, ids, **kwargs):
        """

        """
        selected_actions_msgs = [actions[:, _id, 0, 0].tolist() for _id in range(len(ids))]
        ret = self._exch_msgs(msgs=selected_actions_msgs, ids=[_b for _b in ids], **kwargs)
        return ret
    ...
    def run(self, test_mode, **kwargs):
        ...
        self.N_episode += self.batch_size

        ttype = th.cuda.FloatTensor if self.episode_buffer.is_cuda else th.FloatTensor
        if "__n_episode" in self.episode_buffer.columns._episode: # MAGIC number
            self.episode_buffer.set_col(col="__n_episode",
                                        scope="episode",
                                        data=ttype(self.batch_size, 1).fill_(self.N_episode))
        if "__t_env" in self.episode_buffer.columns._episode: # MAGIC number
            self.episode_buffer.set_col(col="__t_env",
                                        scope="episode",
                                        data=ttype(self.batch_size, 1).fill_(self.T_env))
    ...
    def _add_episode_stats(self, T_env, **kwargs):
        ...
        if self.args.obs_noise and self.test_mode: # evil hack
        ...
        log_hdf = kwargs.get("log_hdf", False)
        if self.test_mode or log_hdf:
            if hasattr(self.args, "save_episode_samples") and self.args.save_episode_samples:
                assert self.args.use_hdf_logger, "use_hdf_logger needs to be enabled if episode samples are to be stored!"
                self.logging_struct.hdf_logger.log(test_suffix, self.episode_buffer, self.T_env)
    ...
    def log(self,log_directly = True, **kwargs):
        ...
        """
        test_suffix = "" if not self.test_mode else "_test"
        if self.args.obs_noise and self.test_mode: # bad hack
            obs_noise_std = kwargs.get("obs_noise_std", 0.0)
            test_suffix = "_noise{}_test".format(obs_noise_std)
